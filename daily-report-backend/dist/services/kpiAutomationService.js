...
const hasScorableAutoEvidence = (values) => values.some((value) => value !== null && value !== undefined);
...
const getBlockingBugLinks = (issue) => (issue.linkedIssues || []).filter((link) => {
...
const isExcusedByBlockingBug = (issue, dueDate, actualEndAt) => {
    const blockingBugLinks = getBlockingBugLinks(issue);
    if (!blockingBugLinks.length)
        return false;
    const due = endOfDueDate(dueDate);
    if (!due)
        return false;
...
const computeImplementationDomain = (issues, npsEntries, period, implParentEpicKeys = new Map()) => {
    ...
    const epicKeyToParentTasks = new Map();
    for (const issue of relevant) {
        if (!issue.parentKey)
            continue;
        const epicKey = implParentEpicKeys.get(issue.parentKey) || issue.parentKey;
        const parentTasks = epicKeyToParentTasks.get(epicKey) || new Set();
...
const computeEngineerDeliveryKpi = async (profile, user, period, storedScorecard, npsEntries = []) => {
    ...
    try {
        ...
        pmParents = parentKeys.length
            ...
            : [];
        const implParentKeys = Array.from(new Set(subtasks
    ...
    const implParentEpicKeys = new Map((implParents || []).map((issue) => [issue.key, issue.parentKey || null]));
